...
router.put('/:device_id',(req,res)=>{
    const{device_id,registration_number,device_number,center_number,
        vehical_model,service,vehicle_type,customer_gmail,is_approve}=req.body
...
router.get("/:id/connected",(req,res)=>{
    ...
    Connection.find({dealer_id:mongoose.Types.ObjectId(req.params.id),is_approve:true})
    .populate('dealer') 
    .then(connected=>{return res.status(200).json(connected)})
